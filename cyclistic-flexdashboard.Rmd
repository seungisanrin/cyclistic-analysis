...
```{r creating dataframe, include=FALSE}
...
### Number of Distinct Rides
```{r number of rides}
comma_text <- dollar_format(prefix = "",suffix="")
n_rides <- nrow(clean_cyc_df)
valueBox(
  value = comma_text(n_rides),
  color = "#2b2d42"
  )
```
...
valueBox(
  value = paste(avg_ride_length, " Minutes"),
  color = "#2b2d42"
)
```
...
valueBox(
  value = paste(max_ride_length, " Minutes"),
  color = "#2b2d42"
)
```
...
### Ride Distribution: Member-Type Analysis
```{r member type distribution}
...
marker = list(colors = color_pie)) %>%
  add_pie(hole = 0.42) %>%
  layout(annotations=list(text=paste("Over 4M Rides"),"showarrow" =F))
fig_pi_chart
```

### Weekday Trip Frequency: Member-Type Analysis
```{r number of trips per day per user type}
...
### Average Ride Length: Member-Type Analysis
```{r average ride length per member type}
...
fig_avg_ride_p_mem <- fig_avg_ride_p_mem %>%
  layout(xaxis = list(title = "Type of Membership"),
  yaxis = list(title = "Average Ride Length (Mins)"))

fig_avg_ride_p_mem
```

### Bike Preferences: Member-Type Analysis
```{r bike preferences per member type}
...
ggplot(member_pref_bike_type, mapping = aes(x=rideable_type, y=no_trips, fill=member_casual)) +
  geom_bar(position = "dodge", stat = "identity") +
  labs(y = "Number of Trips", x ="Bike Type") +
  guides(fill=guide_legend(title="User Type")) +
  scale_fill_manual(values = c("#2596BE", "#2b2d42")) +
  theme_minimal()
)
...
### Average Ride Length: Weekday Analysis
```{r average length of rides per day}
...
fig_avg_pdotw <- fig_avg_pdotw %>%
  layout(yaxis = list(title = "Average Ride Length (Mins)"))
...
Row
--------------------------------------------------------------------------------
### Ride Frequency Analysis: Weekday Basis
```{r ride frequency weekday analysis}
...
### Ride Frequency Analysis: Hourly Basis
```{r number of trips every hour of the day}
...
Row
--------------------------------------------------------------------------------
### Average Ride Length: Customer-Bike Analysis
```{r average ride length analysis}
...
ggplot(avg_rl_bt_ct, mapping = aes(x=rideable_type, y=avg_ride_length, fill=member_casual)) +
  geom_bar(position = "dodge", stat = "identity") +
  labs(y = "Average Ride Length (Mins)", x ="Bike Type") +
  guides(fill=guide_legend(title="User Type")) +
  scale_fill_manual(values = c("#2596BE", "#2b2d42")) +
  theme_minimal()
)
...
### Top 5 Most Visited Starting Station: Casual Riders
```{r top five starting Stations for casual}
...
fig_top_5_sscas <- plot_ly(top_5_start_stations_cas, x=~no_trips, y=~start_station_name, type = "bar", colors = "#2596BE") %>%
  layout(xaxis = list(title = "Number of Trips"),
    yaxis = list(title = "Station Name"))
fig_top_5_sscas
```

### Top 5 Most Visited Starting Stations: Member Riders
```{r top five starting Stations for members}
...
Row 
--------------------------------------------------------------------------------
### Monthly Bike Preference: Member-Type Basis
```{r monthly bike preference}
...
ggplot(bp_p_month, mapping = aes(x=no_trips, y=ride_month, fill=member_casual)) +
  ...
  labs(y = "Month", x ="Number of Trips") +
  guides(fill=guide_legend(title="User Type")) +
  scale_fill_manual(values = c("#2596BE", "#2b2d42")) +
